...
- **2 or 4 clickable responses**, so that is symetric. Some events like "go or not go" type of decisions, and some more open like "what do you want to do?" type of decisions, obviously, more creative and fun decisions.
- **Hidden Consequences:** The UI will NOT display which stats will be affected before clicking. The player must read the context and intuitively weigh the real-world risk of their actions, so that the player has to think about the consequences of their actions. (e.g., deciding to skip class to party might intuitively hurt Grades and boost Social, but the buttons will only show narrative actions, not raw numbers).
